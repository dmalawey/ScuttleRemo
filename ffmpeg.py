# Modifications (last updated 2019.11.18 with successful streaming)
# mods are in lines 256 thru 260, including frame rates of 12.5, and setpts item
#------------------------------------------------------------------------------
...
#    server refresh
    ...
    if networking.internetStatus:
       ...
       videoCommandLine = ('{ffmpeg} -f {input_format} -framerate 12.5 -video_size {xres}x{yres}'
                        ' -r 12.5 {in_options} -i {video_device} {video_filter}'
                        ' -f mpegts -crf 51 -codec:v {video_codec} -b:v {video_bitrate}k -bf 0'
                        ' -muxdelay 0.001 {out_options} -filter:v "setpts=0.5*PTS,fps=25"'
